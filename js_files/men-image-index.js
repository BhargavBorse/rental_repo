...
dbRef.child('item').child('Women').on('value',function(item_details_snapshot){
    ...
    for(var i=0;i<women_keys.length;i++)
    {
        itemRef.child('Women').child(women_keys[i]).on('value',function(item_deep_details_snapshot){
            ...
            // div_subMinus2.className="owl-stage-outer";

            // //Appending div class="owl-stage-outer" to owl-demo (Main Div)
            // document.getElementById('owl-demo').appendChild(div_subMinus2);

            // //Div class=owl-stage CHECK in Responsive Mode for more Info.
            // var div_subMinus1 = document.createElement('div');
            // div_subMinus1.className="owl-stage";
            // div_subMinus1.setAttribute('style','transform: translate3d(-922px, 0px, 0px); transition: all 0.25s ease 0s; width: 3227px;');

            // //Appending div class=owl-stage to div-class=owl-stage-outer
            // div_subMinus2.appendChild(div_subMinus1);
            ...
            alink.setAttribute('href', 'checkout.html?item=' + women_keys[i]);
